...
test("leases store liveness metadata and renew atomically with mode 0600", async () => {
  ...
  assert.deepEqual((await readdir(join(root, ".github-ops", "locks"), { recursive: true })).filter((name) => name.endsWith(".tmp")), []);
});
...
test("a crashed dead mutation guard is recovered safely", async () => {
  ...
  const guard = join(root, ".github-ops", "locks", `${digest}.lock.guard`);
...
async function findLease(root) {
  const locks = join(root, ".github-ops", "locks");
