...
test("legacy work plans migrate to exact conservative v4 golden results", async () => {
  for (const [inputName, expectedName] of [
    ...
    ["valid-work-plan.json", "work-plan-v2-to-v4.json"],
  ]) {
...
test("handoff v1 migrates to the exact conservative v2 golden result", async () => {
  const input = await readJson(fixture("legacy-handoff-v1.json"));
...
test("migration does not invent decomposition scope or legacy priority", async () => {
  const input = await readJson(fixture("valid-work-plan.json"));
...
test("migration diagnoses mixed legacy terminal boundaries", async () => {
  const input = await readJson(fixture("valid-work-plan.json"));
...
test("migration eligibility accepts project-defined role slugs", async () => {
  const input = await readJson(fixture("valid-work-plan-v4.json"));
  input.issues[0].ownerRole = "legal-counsel";
  input.issues[0].delivery.ownerRole = "legal-counsel";
  const result = migrateWorkPlan(input);
...
test("local rollback restores unchanged output and refuses concurrent edits", async () => {
  ...
  const beforeArtifact = await readJson(fixture("valid-work-plan.json"));
  ...
  const snapshotPath = await writeMigrationSnapshot(plan, { directory: join(directory, ".github-ops", "migrations") });
...
test("migration CLI previews, applies, checks rollback, and restores locally", async () => {
  ...
  const input = await readJson(fixture("legacy-work-plan-eligible.json"));
...
test("mixed terminal boundaries survive re-preview instead of being laundered", async () => {
  const plan = JSON.parse(await readFile(join(root, "tests", "fixtures", "valid-work-plan.json"), "utf8"));
  plan.issues[1].delivery = {
    mode: "software-merge",
