...
GitHub has no native Initiative object; a goal that spans multiple Projects stays
a documented objective in each Project's description. A `goal-structure` work plan
may create one Project, ordered logical phases, repository milestones, parent
outcome issues, blocking decisions, and durable resources. It must not create
execution tasks.
Logical phases are contract metadata in the Project description or approved
lifecycle resource, not fabricated GitHub objects.
